...
export default function DailySiteReports() {
  ...
  function activityNotesFromTimeEntries(nextProjectId, reportDate = date) {
    const relevantEntries = entries.filter((e) => String(e.work_date).slice(0, 10) === reportDate);
    const relevant = relevantEntries.filter((e) => String(e.project_id || "") === String(nextProjectId) && !e.absence_type && !isBadWeatherEntry(e));
    return [...new Set(relevant.map((row) => String(row.note || "").trim()).filter(Boolean))].join("\n");
  }
  ...
  function prepareProject(nextProjectId) {
    ...
    setEmployeeItems(employeeItemsFromTimeEntries(nextProjectId, date)); setActivities(activityNotesFromTimeEntries(nextProjectId, date)); setWeather(weatherText);
  ...
  function refreshActivitiesFromTimeEntries() {
    if (!projectId || status === "completed") return;
    const notes = activityNotesFromTimeEntries(projectId, date);
    setActivities(notes);
    setMessage(notes ? "Notizen aus der Zeiterfassung als Vorlage übernommen." : "Keine Notizen in der Zeiterfassung gefunden. Das Feld darf leer bleiben.");
    setError("");
  }
  ...
  async function createPdfDocument() {
    ...
    await addPdfWatermark(doc, { opacity: 0.18, size: 470, yOffset: 18 });
