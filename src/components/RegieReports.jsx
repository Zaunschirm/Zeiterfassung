...
export default function RegieReports() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const [projectResult, employeeResult, timeEntryResult, reportResult, templateResult, billingResult] = await Promise.all([
        ...
        supabase.from("project_billing_records").select("project_id,billing_data"),
      ]);
      ...
      if (billingResult.error) console.warn("Regie-Abrechnungsstatus konnte nicht geladen werden.", billingResult.error);
      ...
      setBillingByProject((billingResult.data || []).reduce((map, record) => {
        map[String(record.project_id)] = Array.isArray(record.billing_data?.regieBilledIds) ? record.billing_data.regieBilledIds.map(String) : [];
        return map;
      }, {}));
    } catch (e) {
  ...
  async function exportReportsPdf(reportsToExport, fileLabel = "Auswahl") {
    ...
    try {
      ...
      const autoTable = autoTableModule.default;
      const brown = PDF_BRAND.brown;
      const reportsForPdf = [...reportsToExport].sort((a, b) => {
  ...
  return (
    <div className="hbz-container regie-page">
        ...
        <aside className="hbz-card regie-list">
          ...
          {showArchived && canPrepare && groupedArchiveReports.map((group) => (
            <section className="regie-archive-project" key={group.key}>
              <div className="regie-archive-project-head"><b>{group.label}</b><div><span>{group.reports.length}</span><button type="button" className="hbz-btn btn-small" disabled={busy || !group.reports.length} onClick={() => exportReportsPdf(group.reports, group.label)}>Projekt als PDF speichern</button></div></div>
              ...
              <section className="regie-section">
                ...
                {materialItems.map((row, index) => (
                  <div className="regie-row material" key={index}>
                    <input className="hbz-input regie-material-description" value={row.description} onChange={(e) => setMaterialItems((rows) => rows.map((item, i) => i === index ? { ...item, description: e.target.value } : item))} placeholder="Material / Gerät" />
      ...
      <style>{`
        ...
        @media(max-width:800px){.regie-header{align-items:stretch;flex-direction:column}.regie-layout{grid-template-columns:1fr}.regie-list{position:static;max-height:230px;overflow:auto}.regie-grid,.regie-assignment-grid{grid-template-columns:1fr}.regie-row.labor{grid-template-columns:1fr 96px 42px}.regie-row.material{grid-template-columns:1fr 76px 92px 42px}.regie-form{padding:13px}.regie-actions{position:sticky;bottom:0;background:rgba(255,255,255,.96);padding:10px 0 calc(10px + env(safe-area-inset-bottom));z-index:4;max-height:42vh;overflow:auto;box-shadow:0 -8px 22px rgba(70,43,29,.09)}.regie-actions .hbz-btn{flex:1 1 150px;min-height:44px}.regie-signature-canvas{height:150px}.regie-signature-overlay{padding:0}.regie-signature-modal{height:100dvh;border-radius:0;padding:10px}.regie-signature-head{align-items:flex-start;flex-wrap:wrap}.regie-signature-actions{padding-bottom:env(safe-area-inset-bottom);display:grid;grid-template-columns:1fr 1fr}.regie-signature-actions .hbz-btn{width:100%}.regie-photo-upload{width:100%;justify-content:center;min-height:44px}.regie-section-head{align-items:flex-start;flex-wrap:wrap}.regie-photo-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.regie-pdf-overlay{padding:0}.regie-pdf-modal{height:100vh;border-radius:0}}@media(max-width:430px){.regie-row.labor{grid-template-columns:1fr 90px 40px}.regie-row.material{grid-template-columns:1fr 88px 40px}.regie-row.material .regie-material-description{grid-column:1/-1}.regie-total{text-align:left}.regie-photo-grid{grid-template-columns:1fr}}
      `}</style>
    </div>
  );
}
