...
export default function RegieReports() {
  ...
  const assignedEmployeeNames = useMemo(() => assignedEmployeeIds.map((id) => employees.find((employee) => String(employee.id) === String(id))?.name || id).filter(Boolean), [assignedEmployeeIds, employees]);
      ...
      <div className="regie-layout">
        ...
        {hasOpenReport ? (
          <main className="hbz-card regie-form">
            <div className="regie-form-head"><div><div className="eyebrow">{isArchived ? "Archiv" : locked ? "Abgeschlossen" : canPrepare ? "Desktop-Vorbereitung" : "Handy-Erfassung"}</div><h2>{reportNumber}</h2>{canPrepare && status !== "draft" && <p className="regie-release-info">Freigegeben für: {assignedEmployeeNames.length ? assignedEmployeeNames.join(", ") : "keine Mitarbeiter ausgewählt"}</p>}</div><span className={`regie-status ${isArchived ? "archived" : status}`}>{statusLabel}</span></div>
