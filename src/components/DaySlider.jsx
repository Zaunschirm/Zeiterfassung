...
// BUAK Kurz-/Langwoche (2026) – direkt hier, damit DaySlider 100% stabil bleibt (kein extra Import)
...
function getBuakWeekInfo(dateStr) {
  const wk = isoWeekNumber(dateStr);
  if (!wk) return { label: "", type: null };

  const year = Number(String(dateStr).slice(0, 4));
  if (year !== 2026) return { label: `KW ${wk}`, type: null };

  const t = BUAK_WEEK_TYPES_2026[wk];
  if (!t) return { label: `KW ${wk}`, type: null };

  const type = t === "K" ? "kurz" : "lang";
  const soll = type === "kurz" ? 36 : 42;
  const txt = type === "kurz" ? "Kurzwoche" : "Langwoche";
  return { label: `KW ${wk} · ${txt} (${soll}h)`, type };
}
...
const hmToMin = (hm) => {
  ...
  // BUAK Anzeige (stabil)
  const buakInfo = getBuakWeekInfo(date);
