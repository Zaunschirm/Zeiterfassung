...
// Simple token (not full JWT to keep minimal). Signed via HMAC-like approach.
...
function ensureDir(p){ try{ require('fs').mkdirSync(p, {recursive:true}); } catch{} }
ensureDir(dataDir);
...
if (process.argv.includes('--init-db')){
  runSchema();
  seed();
  console.log('DB initialisiert mit Demo-Usern.');
  process.exit(0);
}
...
function setAuthCookie(res, session){
  ...
  res.cookie('session', sign(session), {
    httpOnly: true,
    secure: isProd,            // in production we set secure
...
app.post('/api/admin/impersonate', requireAuth, requireRole('ADMIN'), (req,res)=>{
  ...
  const session = { userId: target.id, impersonatedUserId: req.user.id };
  setAuthCookie(res, session);
...
app.get('/api/timesheets', requireAuth, (req,res)=>{
  ...
  // Teamleiter darf nur sein Team sehen
  ...
  const rows = db.prepare('SELECT * FROM timesheets WHERE user_id=? AND substr(date,1,7)=? ORDER BY date')
    .all(uid, month);
  res.json({items: rows});
});
...
app.post('/api/timesheets', requireAuth, (req,res)=>{
  ...
  // Mitarbeiter darf nur für sich selbst schreiben, Admin für alle, Teamleiter für sein Team
...
// Nur Admin darf Einträge bearbeiten (Monatsübersicht editierbar nur für Admin)
app.put('/api/timesheets/:id', requireAuth, requireRole('ADMIN'), (req,res)=>{
  const id = Number(req.params.id);
...
// Password change enforcement (for SPA routes, return flag)
